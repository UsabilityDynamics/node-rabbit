...
function RabbitClient() {
  ...
  try {
    ...
    context.get( 'connection' ).once( 'ready', function get_exchanges() {
...
Object.defineProperties( RabbitClient.prototype, {
  ...
  connect: {
    ...
    value: function connect() {
      ...
      this.get( 'connection' ).once( 'ready', function get_exchanges() {
        context.debug( 'Connection is ready.' );

        // Register Activity Exchange Document
        RabbitClient.Activity.declare( 'activity', {
        ...
        RabbitClient.Session.declare( 'session', {
          connection: context.get( 'connection' ),
          durable: true,
          confirm: false,
          passive: false,
        ...
        RabbitClient.Session.declare( 'job', {
          connection: context.get( 'connection' ),
          durable: true,
          confirm: false,
          passive: false,
  ...
  processJob: {
    ...
    value: function processJob( name, message, handler ) {
      ...
      // Create or load Activity.
      RabbitClient.Job.create( 'unique-id-of-this-task' ).subscribe( '', function() {
