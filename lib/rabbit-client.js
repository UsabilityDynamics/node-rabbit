 ...
 * @returns {RabbitClient} Newly created Rabbit instance.
 * @version 0.1.0
 *
 * @async
 * @chainable
...
function RabbitClient() {
  ...
  var handler   = 'function' === typeof arguments[1] ? arguments[1] : RabbitClient.utility.noop;
  ...
  RabbitClient.utility.emitter.mixin( self);
  RabbitClient.utility.settings.mixin( self  );
  ...
  // Configure instance.
  self.set({
    settings: require( '../package' ).config,
    options: {
      job_subscription: {
        prefetchCount: 10,
        ack: true
  ...
  // Connect to AMQP.
  this.connection = RabbitClient.amqp.createConnection({
  ...
  // RabbitClient connection failed.
